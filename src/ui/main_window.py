...
class MainWindow(QMainWindow):
    ...
    def _add_new_path_to_active(self):
        ...
        if directory:
            self.move_path_to_active(directory)
    
    def _handle_delete_from_history(self, path: str):
        reply = QMessageBox.question(self, "Confirmar Eliminación", 
            f"¿Está seguro de que desea eliminar '{path}' del historial? Se borrarán todos los datos cacheados de esta ruta.",
    ...
    def _start_scan(self):
        ...
        self.worker.signals.status_update.connect(self._update_status); self.worker.signals.progress.connect(self.progress_bar.setValue)
        self.worker.signals.finished.connect(self._scan_finished); self.worker.signals.results_ready.connect(self._populate_results_area)
    ...
    def _populate_results_area(self, duplicate_structure: dict):
        ...
        for series_title, duplicate_episodes in sorted(series_found.items()):
            ...
            series_widget.ignore_requested_signal.connect(self._handle_ignore_request)
        ...
        if movies_found:
            ...
            for movie_group in sorted(movies_found, key=lambda g: g.display_title):
                # La lógica de ignorar películas necesitaría un ID estandarizado similar
                movie_widget = DuplicateGroupWidget(movie_group, standardize_text(movie_group.display_title))
    ...
    def _handle_ignore_request(self, key: str, level: str):
        reply = QMessageBox.question(self, "Confirmar Ignorar",
            f"¿Desea ignorar '{key}' en futuros escaneos? Esta acción se puede revertir editando la base de datos.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self.cache.add_to_ignore_list(key, level)
            self.scan_button.click() # Refrescar la vista re-escaneando